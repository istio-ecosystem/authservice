...
| not_strict_https | This option is False by default. When set to True, it allows non-HTTPS schemes in the configured URI (e.g. callback URI or Authorization URI). It is not recommended to set this option to False and use it in production, but it can be useful for quick debugging. Optional. | bool |
