...
| healthz_listen_port | The TCP port for the Authservice to listen for incorming REST request for healthcheck. Healthcheck is used to check not only for checking that server is up and running but valid JWKs for token validation are extracted and available for request validation. Required. | int32 |
