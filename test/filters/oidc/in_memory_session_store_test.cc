 ...
 protected:
  ...
  void SetUp() override {
    auto jwt_status = id_token_jwt.parseFromString(
        ...
        "lhdCI6MTUxNjIzOTAyMiwiYXVkIjpbImNsaWVudDEiXSwibm9uY2UiOiJyYW5kb20ifQ."
        "NQi_"
        "VTRjZ8jv5cAp4inpuQ9STfVgCoWfONjLnZEMk8la8s99J9b6QmcKtO2tabTgvcseikVNlP"
        "uB6fZztY_fxhdrNE0dBNAl1lhz_AWBz6Yr-D82LLKk5NQ-"
...
TEST_F(InMemorySessionStoreTest, SetTokenResponseAndGetTokenResponse) {
  ...
  // Caution: when you mutate the original, you mutate the same object that is
  // held in the session store's map
  ...
  ASSERT_EQ(result->AccessToken(),
            "fake_access_token2");  // will be the mutated value, so be careful!
  ASSERT_EQ(result->GetAccessTokenExpiry(), 42);

  token_response->SetAccessTokenExpiry(99);
  in_memory_session_store.SetTokenResponse(session_id,
                                           token_response);  // overwrite
...
TEST_F(
    InMemorySessionStoreTest,
    SetAuthorizationStateAndClearAuthorizationStateAndGetAuthorizationState) {
  InMemorySessionStore in_memory_session_store(time_service_mock_, 42, 128);
  ...
  in_memory_session_store.ClearAuthorizationState(
      session_id);  // does not crash
  ASSERT_FALSE(result);

  in_memory_session_store.SetAuthorizationState(session_id,
                                                authorization_state);

  result = in_memory_session_store.GetAuthorizationState(other_session_id);
  ASSERT_FALSE(result);

  // mutate original strings that were passed to AuthorizationState constructor,
  // make sure it doesn't change AuthorizationState
  nonce += "-modified";
  state += "-modified";
  ...
  auto another_authorization_state = std::make_shared<AuthorizationState>(
      another_state, another_nonce, another_requested_url);
  in_memory_session_store.SetAuthorizationState(
      session_id, another_authorization_state);  // overwrite
...
TEST_F(InMemorySessionStoreTest, Remove) {
  ...
  in_memory_session_store.RemoveSession(
      "other-session-id");  // ignore non-existent keys without error
}

TEST_F(
    InMemorySessionStoreTest,
    RemoveAllExpired_RemovesSessionsWhichHaveExceededTheMaxAbsoluteSessionTimeout) {
  ...
  InMemorySessionStore in_memory_session_store(
      time_service_mock_, absolute_session_timeout_in_seconds,
  ...
  in_memory_session_store.SetTokenResponse(session_id_will_expire,
                                           token_response_will_expire);
  ...
  in_memory_session_store.SetTokenResponse(session_id_will_not_expire,
                                           token_response_will_not_expire);
  ASSERT_TRUE(
      in_memory_session_store.GetTokenResponse(session_id_will_not_expire));

  // After 30 seconds, neither should have been cleaned up
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(30));
  in_memory_session_store.RemoveAllExpired();
  ASSERT_TRUE(in_memory_session_store.GetTokenResponse(
      session_id_will_expire));  // has been in for 25 seconds
  ASSERT_TRUE(in_memory_session_store.GetTokenResponse(
      session_id_will_not_expire));  // has been in for 10 seconds

  // After 200 seconds, the older session is cleand up but the younger one is
  // not
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
...
TEST_F(
  ...
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(1501));
  in_memory_session_store.RemoveAllExpired();
  ASSERT_FALSE(in_memory_session_store.GetTokenResponse(
      session_id));  // removed due to idle timeout, don't care about time since
                     // added
}
...
TEST_F(
  ...
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(1004));
  ASSERT_TRUE(in_memory_session_store.GetTokenResponse(session_id));

  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(1006));
  in_memory_session_store.RemoveAllExpired();
  ASSERT_FALSE(in_memory_session_store.GetTokenResponse(
      session_id));  // removed due to max absolute timeout, even though it was
                     // just accessed
}

TEST_F(InMemorySessionStoreTest,
       RemoveAllExpired_DoesNotEverRemoveSessionsWhenBothTimeoutsAreZero) {
...
TEST_F(
    InMemorySessionStoreTest,
    RemoveAllExpired_RemovesSessionsWhichHaveExceededTheMaxIdleSessionTimeout) {
  ...
  in_memory_session_store.SetTokenResponse(session_id_idle,
                                           token_response_idle);
  ...
  in_memory_session_store.SetTokenResponse(session_id_active,
                                           token_response_active);
  ...
  in_memory_session_store.RemoveAllExpired();
  ASSERT_TRUE(in_memory_session_store.GetTokenResponse(
      session_id_idle));  // last active 25 seconds ago
  ASSERT_TRUE(in_memory_session_store.GetTokenResponse(
      session_id_active));  // last active 25 seconds ago

  // Access only one of two at time 50
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(50));
  ASSERT_TRUE(in_memory_session_store.GetTokenResponse(
      session_id_active));  // accessing at time 50
  ...
  in_memory_session_store.RemoveAllExpired();
  ASSERT_FALSE(in_memory_session_store.GetTokenResponse(
      session_id_idle));  // last active 60 seconds ago
  ASSERT_TRUE(in_memory_session_store.GetTokenResponse(
      session_id_active));  // last active 40 seconds ago
}

TEST_F(InMemorySessionStoreTest,
       RemoveAllExpired_UpdatingTokenResponseKeepsSessionActive) {
  InMemorySessionStore in_memory_session_store(time_service_mock_, 500, 50);
  ...
  in_memory_session_store.SetTokenResponse(
      session_id_idle, updated_token_response);  // last active 25 seconds ago
  in_memory_session_store.SetTokenResponse(
      session_id_active, updated_token_response);  // last active 25 seconds ago

  // Access only one of two at time 50
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(50));
  in_memory_session_store.SetTokenResponse(
      session_id_active, updated_token_response);  // accessing at time 50
...
TEST_F(InMemorySessionStoreTest,
       RemoveAllExpired_UpdatingAuthorizationStateKeepsSessionActive) {
  InMemorySessionStore in_memory_session_store(time_service_mock_, 500, 50);
  auto authorization_state =
      std::make_shared<AuthorizationState>("state", "nonce", "requested_url");
  ...
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(5));
  auto session_id_idle = std::string("fake_session_id_idle");
  in_memory_session_store.SetAuthorizationState(session_id_idle,
                                                authorization_state);
  ASSERT_TRUE(in_memory_session_store.GetAuthorizationState(session_id_idle));
  auto session_id_active = std::string("fake_session_id_active");
  in_memory_session_store.SetAuthorizationState(session_id_active,
                                                authorization_state);
  ...
  in_memory_session_store.RemoveAllExpired();
  in_memory_session_store.SetAuthorizationState(
      session_id_idle, authorization_state);  // last active 25 seconds ago
  in_memory_session_store.SetAuthorizationState(
      session_id_active, authorization_state);  // last active 25 seconds ago

  // Access only one of two at time 50
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(50));
  in_memory_session_store.SetAuthorizationState(
      session_id_active, authorization_state);  // accessing at time 50
...
TEST_F(InMemorySessionStoreTest,
       RemoveAllExpired_ClearAuthorizationStateKeepsSessionActive) {
  InMemorySessionStore in_memory_session_store(time_service_mock_, 500, 50);
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(5));
  auto authorization_state =
...
TEST_F(
    InMemorySessionStoreTest,
    RemoveAllExpired_RemovesAuthorizationStatesWhichHaveExceededTheAbsoluteTimeout) {
  int absolute_session_timeout_in_seconds = 190;
  int idle_session_timeout_in_seconds = 0;
  auto authorization_state1 = std::make_shared<AuthorizationState>(
      "state1", "nonce1", "requested_url1");
  auto authorization_state2 = std::make_shared<AuthorizationState>(
      "state2", "nonce2", "requested_url2");
  InMemorySessionStore in_memory_session_store(
      time_service_mock_, absolute_session_timeout_in_seconds,
      idle_session_timeout_in_seconds);
  ...
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(5));
  auto session_id_will_expire = std::string("fake_session_id_1");
  in_memory_session_store.SetAuthorizationState(session_id_will_expire,
                                                authorization_state1);
  ASSERT_TRUE(
      in_memory_session_store.GetAuthorizationState(session_id_will_expire));

  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(20));
  auto session_id_will_not_expire = std::string("fake_session_id_2");
  in_memory_session_store.SetAuthorizationState(session_id_will_not_expire,
                                                authorization_state2);
  ASSERT_TRUE(in_memory_session_store.GetAuthorizationState(
      session_id_will_not_expire));
  ...
  // After 200 seconds, the older session is cleaned up but the younger one is
  // not
  EXPECT_CALL(*time_service_mock_, GetCurrentTimeInSecondsSinceEpoch())
      .WillRepeatedly(Return(200));
  in_memory_session_store.RemoveAllExpired();
...
TEST_F(
  ...
  InMemorySessionStore in_memory_session_store(time_service_mock_, 500, 50);
  auto authorization_state_idle = std::make_shared<AuthorizationState>(
      "state1", "nonce1", "requested_url1");
  auto authorization_state_active = std::make_shared<AuthorizationState>(
      "state2", "nonce2", "requested_url2");
  ...
  for (int i = 0; i < thread_count; ++i) {
    threads.emplace_back([iterations, i, &in_memory_session_store, this]() {
      for (int j = 1; j < iterations + 1; ++j) {
        ...
        auto retrieved_optional_token_response =
            in_memory_session_store.GetTokenResponse(key);

        ASSERT_TRUE(retrieved_optional_token_response);
        ASSERT_TRUE(retrieved_optional_token_response->GetAccessTokenExpiry()
                        .has_value());
        ASSERT_EQ(
            retrieved_optional_token_response->GetAccessTokenExpiry().value(),
            unique_number);
      }
    });
  }
  ...
  for (int i = 0; i < thread_count; ++i) {
    threads.emplace_back([iterations, i, &in_memory_session_store]() {
      for (int j = 1; j < iterations + 1; ++j) {
        ...
        ASSERT_EQ(retrieved_token_response->GetAccessTokenExpiry().value(),
                  unique_number);
...
TEST_F(InMemorySessionStoreTest, ThreadSafetyForClearAuthorizationState) {
  ...
  // Do lots of set() and clear() at the same time that calls to remove() are
  // happening, all on the same session. Eventually clear() will be called on a
  // session that has already been removed and cause a crash, unless clear() is
  // properly synchronized.
  for (int i = 0; i < thread_count; ++i) {
    threads.emplace_back([iterations, &in_memory_session_store]() {
      for (int j = 1; j < iterations + 1; ++j) {
        auto authorization_state = std::make_shared<AuthorizationState>(
            "state", "nonce", "requested_url");
        in_memory_session_store.SetAuthorizationState("session_id",
                                                      authorization_state);
        in_memory_session_store.ClearAuthorizationState("session_id");
      }
...
TEST_F(InMemorySessionStoreTest, ThreadSafetyForGetAuthorizationState) {
  ...
  // Do lots of set() and get() at the same time that calls to remove() are
  // happening, all on the same session. Eventually get() will be called on a
  // session that has already been removed and cause a crash, unless get() is
  // properly synchronized.
  for (int i = 0; i < thread_count; ++i) {
    threads.emplace_back([iterations, &in_memory_session_store]() {
      for (int j = 1; j < iterations + 1; ++j) {
...
TEST_F(InMemorySessionStoreTest, ThreadSafetyForSetAuthorizationState) {
  ...
  for (int i = 0; i < thread_count; ++i) {
    // Each thread has its own instance of URL because otherwise the threads
    // clobber the URL
    threads.emplace_back([iterations, i, &in_memory_session_store]() {
      for (int j = 1; j < iterations + 1; ++j) {
        int unique_number = (i * iterations) + j;
        auto key = std::string("session_id_") + std::to_string(unique_number);
        auto unique_url =
            std::string("https://example.com") + std::to_string(unique_number);
        auto authorization_state =
            std::make_shared<AuthorizationState>("state", "nonce", unique_url);

        in_memory_session_store.SetAuthorizationState(key, authorization_state);

        auto retrieved_optional_authorization_state =
            in_memory_session_store.GetAuthorizationState(key);
