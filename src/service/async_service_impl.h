...
::grpc::Status Check(const RequestType& request, ResponseType& response,
  ...
  try {
    auto request_path = common::http::PathQueryFragment(request_v3.attributes().request().http().path()).Path();
    ...
    if (!common::utilities::trigger_rules::TriggerRuleMatchesPath(request_path, trigger_rules_config)) {
      spdlog::debug(
          ...
          __func__,
          request_v3.attributes().request().http().scheme(), request_v3.attributes().request().http().host(),
    ...
    for (auto &chain : chains) {
      if (chain->Matches(&request_v3)) {
        ...
        if constexpr (std::is_same_v<ResponseType, ::envoy::service::auth::v2::CheckResponse>) {
          try {
        ...
        } else if (std::is_same_v<ResponseType, ::envoy::service::auth::v3::CheckResponse>) {
