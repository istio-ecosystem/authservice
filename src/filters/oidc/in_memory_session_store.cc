...
public:
  ...
  inline void SetAuthorizationState(std::shared_ptr<AuthorizationState> authorization_state) {
...
Session::Session(uint32_t time_added)
    : token_response_(nullptr), authorization_state_(nullptr), time_added_(time_added),
