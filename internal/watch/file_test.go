...
func TestFileWatcher(t *testing.T) {
	testCases := []struct {
		...
		{desc: "watch single file but do not trigger modify",
			numFiles: 1, opts: NewOpts(WithFallbackTimeout(time.Millisecond), WithCheckInterval(time.Millisecond)), skipModify: true},
		{desc: "watch multiple files but do not trigger modify",
			numFiles: maxFileWatcherTestFiles, opts: NewOpts(WithFallbackTimeout(time.Millisecond), WithCheckInterval(time.Millisecond)), skipModify: true},
